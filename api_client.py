            ...
            try:
                async with session.post(API_URL, json=payload) as response:
                    if response.status == 200:
                        ...
                        if attempt < MAX_RETRIES - 1:
                            log.warning(f"Server error {response.status}, retrying in {delay}s (attempt {attempt + 1}/{MAX_RETRIES})")
                ...
                last_exception = e
                if attempt < MAX_RETRIES - 1:
                    log.warning(f"Network error: {e}, retrying in {delay}s (attempt {attempt + 1}/{MAX_RETRIES})")
                    await asyncio.sleep(delay)
                    delay = min(delay * 2, MAX_DELAY)
                    continue
                ...
                if attempt < MAX_RETRIES - 1:
                    log.warning(f"Timeout, retrying in {delay}s (attempt {attempt + 1}/{MAX_RETRIES})")
                    await asyncio.sleep(delay)
                    delay = min(delay * 2, MAX_DELAY)
                    continue
